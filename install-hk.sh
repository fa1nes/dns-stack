...
7 */6 * * * flock -w 550 $LOCK_FILE $GO_BIN classify publish --defer-not-ready >>$LOG_DIR/publish.log 2>&1 || echo "\$(date -Iseconds) publish 未完成：等锁 550s 超时，或发布本身失败" >>$LOG_DIR/publish.log
...
log_ok "日志截断已交给 dns-stack Go 子命令，每日保留每份日志末尾 $((LOG_KEEP_BYTES/1024/1024))MB"
