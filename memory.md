...
⚠️⚠️ **开了 cron 之后原样跑一次，才发现那行本身是坏的**：它用 `flock -n`，
而 `classify pipeline` 每 5 分钟占同一把锁。抢不到锁时 `flock -n` **退出码 1、
一个字都不写**——publish.log 是空的，`status` 只会显示「从未运行过」，
你永远不知道它是没排程、还是每次都被跳过。6 小时一次的任务撞上 5 分钟一次的任务，
被饿死是常态而非意外。

同一个作业的 systemd 单元用的是 `flock -w 550`（等锁），cron 用 `-n`——
**两种调度方式对同一个作业的语义从一开始就不一致**，只是没人跑过 cron 这条路。
已统一成 `-w 550`，并在失败时往 publish.log 写一行时间戳，
把「静默跳过」变成「看得见的跳过」。
`pipeline` / `verify-rules` 保留 `-n` 是对的：它们是「上一轮还没跑完就别排队」。
**只有必须最终发生的作业才该等锁。**

验收方式很重要：不是读那行 cron 觉得对，是**把它原样执行一遍**看退出码和日志。
第一次执行退出码 1、日志空，才暴露了这个问题。
