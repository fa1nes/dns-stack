...
type BuildStats struct {
	...
	Unidentified   []ipset.Range
	UnidentifiedCN uint64
}
...
func Build(opt BuildOptions) ([]Row, BuildStats, error) {
	...
	err = reader.WalkV4(func(prefix netip.Prefix, record map[string]string) error {
		...
		if zone == "" {
			stats.SkippedNoZone++
			for _, part := range intersections(span.Lo, span.Hi, opt.Direct) {
				stats.Unidentified = append(stats.Unidentified, part)
				stats.UnidentifiedCN += uint64(part.Hi-part.Lo) + 1
			}
			return nil
		}
...
func Render(rows []Row) string {
	...
	for _, row := range rows {
		fmt.Fprintf(&b, "%s,%s,%s\n", cidrutil.FormatAddr4(row.Lo), cidrutil.FormatAddr4(row.Hi), row.Zone)
	}
	return b.String()
}
...
func RenderUnidentified(ranges []ipset.Range, total uint64) string {
	collapsed := ipset.Collapse(ranges)
	var b strings.Builder
	fmt.Fprintf(&b, "# generated by dns-stack routing-data (ecs-zone)\n")
	fmt.Fprintf(&b, "# 这些大陆网段在归属库里查不到省份或运营商，拿不到 ECS 缓存分片\n")
	fmt.Fprintf(&b, "# addresses %d\n", total)
	fmt.Fprintf(&b, "# prefixes %d\n", len(collapsed))
	for _, r := range ipset.New(collapsed).Prefixes() {
